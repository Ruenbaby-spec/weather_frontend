...
export default function Home() {
  ...
  <div className="min-h-screen bg-cover bg-center p-4 flex items-center justify-center">
    <div className="w-md max-w-md space-y-4">
      ...
      onSubmit={async (e) => {
        e.preventDefault();
        const formData = new FormData(e.currentTarget);await handleSearch(formData);
        }}
        >
        <input 
        name="city"
      ...
      {error &&(
        <div className="bg-black/70 backdrop-blur text-white">
          {error}
        </div>
      )}
      {weather && (
        ...
        <div className="grid grid-cols-3 gap-4 mt-5">
          <div  className="text-center">
            ...
            <div className="font-semibold text-red-500">{Math.round(weather.main.feels_like)}</div>
          </div>
          <div className="text-center">
          <Wind className="w-7 h-7 mx-auto text-orange-500"/>
            <div className="mt-2 text-md text-orange-500">Wind's Speed</div>
            <div className="font-semibold text-orange-500">{Math.round(weather.wind.speed)}m/s</div>

          </div>
          <div className="text-center">
          <Droplets className="w-7 h-7 mx-auto text-blue-500"/>
            <div className="mt-2 text-md text-blue-500">Humidity.</div>
            <div className="font-semibold text-blue-500">{Math.round(weather.main.humidity)}%</div>
